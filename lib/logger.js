...
function logCsvRowNames(args:Object) {
    const {showRowIndex, rowA, bufferFunction} = args;
    const t = R.join(',', showRowIndex ? R.prepend('row', rowA) : rowA);
    if (bufferFunction){
        bufferFunction(logComponentsToString(t));
    } else {
...
function logCsvRowValues(args:Object) {
    const {showRowIndex, rowIndex, rowA, bufferFunction} = args;
...
function logPrettyValues(args:Object) {
    ...
    R.pipe(
        ...
        array => {
            if (bufferFunction){
                bufferFunction(logArrayToString(width, false));
            }else{
...
function logMarkdownValues(args:Object) {
    ...
    R.pipe(
        ...
        array => {
            if (bufferFunction){
                bufferFunction(logArrayToString(array));
            }else{
