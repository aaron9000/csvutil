...
function logCsvHeader(args:Object) {
...
function logPrettyRowValues(args:Object) {
    const {rowA, showRowIndex, width, bufferFunction} = args;
    R.pipe(
...
// MD
function logMarkdownHeader(args:Object) {
    const {rowA, showRowIndex, width, bufferFunction} = args;
    const row = showRowIndex ? R.prepend('row', rowA) : rowA;
    logMarkdownRowValues(args, false);
    if (bufferFunction) {
        bufferFunction(logDividerToString(width, row, '|', '|'))
    } else {
        logDivider(width, row, '|', '|');
...
function logMarkdownRowValues(args:Object, escapeValues:Boolean = true) {
    const {rowA, showRowIndex, width, bufferFunction} = args;
    R.pipe(
        showRowIndex ? R.prepend('row') : R.identity,
        array => {
            if (bufferFunction) {
                bufferFunction(logArrayToString(width, escapeValues, '|', ' |', array));
            } else {
                logArray(width, escapeValues, '|', ' |', array);
            }
            return array;
        }
    )(rowA);
...
// Helpers
function logFormattedRowDiff(args:Object, equal:Function, notEqual:Function, start:String, end:String) {
    ...
    R.pipe(
        ...
        R.prepend({text: start, color:Color.GREY}),
        R.append({text: end, color:Color.GREY}),
        components => {
            if (bufferFunction) {
                bufferFunction(logComponentsToString(components));
            } else {
...
export function getLogger(format:String):Object {
    ...
    switch (format) {
        ...
        case OutputFormat.PRETTY:
            return {
                logHeader: logPrettyHeader,
                logDiff: logPrettyRowDiff,
                logValues: logPrettyRowValues,
                logSummary: logPrettySummary
            };
        case OutputFormat.MD:
            return {
                logHeader: logMarkdownHeader,
                logDiff: logMarkdownRowDiff,
                logValues: logMarkdownRowValues,
                logSummary: R.T
            };
